...
public class PlayerActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (player != null) {
                    ...
                    binding.progressLarge.setProgress(mCurrentPosition);
                }
        ...
        binding.imgShuffleButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                activeService.shuffleOn();

            }
        });

        binding.progressLarge.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {

                if (progress == binding.progressLarge.getMax()) {
    ...
    public void nextMain() {
        if (pos < model.size() - 1) {
            ...
            PlayerService.setPos(pos);
            activeService.justLoadNotificationMedia();
        }
    }

    public void previousMain() {
        if (pos > 0) {
            player.stop();
            MediaItem mediaItem = MediaItem.fromUri(model.get(pos - 1).getmData());
            pos = pos - 1;
    ...
    public void btnImageFavorite() {
        if (model.get(pos).getIsLiked() == 0) {
            ...
            binding.imgAddFavoritesButton.setImageResource(R.drawable.favorite_1_dark);
            model.get(pos).setIsLiked(1);
            addDatabaseFav(model.get(pos).getmData());

        } else {

            binding.imgAddFavoritesButton.setImageResource(R.drawable.favorite_0);
    ...
    public void loadData(int position2) {
        ...
        binding.progressLarge.setMax(Integer.parseInt(model.get(position2).getDuration()));
        ...
        Picasso.get()
                .load(model.get(position2).getSongImage())
        ...
        for (int i = 0; i < tinydb.getListString("favorites").size(); i++) {
            ...
            if (tinydb.getListString("favorites").get(i).equals(model.get(pos).getmData())) {
                ...
                model.get(pos).setIsLiked(1);
                binding.imgAddFavoritesButton.setImageResource(R.drawable.favorite_1);
            }
        }
